...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, _, _ = self.actor.compute_log_prob(
                    data=data, 
    ...
    def compute_ref_log_prob(self, data: DataProto):
        ...
        # Get topk from config if needed
        topk = self.config.ref.get("topk_logits", None)
        
        with self.ulysses_sharding_manager:
            data = data.to("cpu")  # data will to device with each micro batch on ref.compute_log_prob
            output, entropy, full_log_probs, topk_info = self.ref_policy.compute_log_prob(
                data=data, 
                calculate_entropy=True,
                topk=topk
            )
            
            tensors = {"ref_log_prob": output}  # response part (for backward compatibility)
            # Teacher entropy is REQUIRED for on-policy distillation with entropy-gated Soft KD
            if entropy is None:
                raise RuntimeError(
                    f"ref_policy.compute_log_prob(calculate_entropy=True) returned entropy=None. "
                    f"This is required for on-policy distillation. "
                    f"Check if ref_policy._forward_micro_batch correctly computes entropy. "
                    f"output shape: {output.shape if output is not None else None}, "
                    f"full_log_probs shape: {full_log_probs.shape if full_log_probs is not None else None}"
                )
            # Debug: Log entropy stats
            if entropy.numel() > 0:
                print(f"[compute_ref_log_prob] ref_entropy shape: {entropy.shape}, "
                      f"mean: {entropy.mean().item():.4f}, "
                      f"max: {entropy.max().item():.4f}, "
                      f"min: {entropy.min().item():.4f}, "
                      f"tokens with entropy > 1.0: {(entropy > 1.0).sum().item()}/{entropy.numel()}")
            tensors["ref_entropy"] = entropy  # (bsz, response_length)
            if full_log_probs is not None:
                tensors["ref_full_log_probs"] = full_log_probs  # full sequence log probs
            if topk_info is not None:
                topk_log_probs, topk_indices = topk_info
                tensors["ref_topk_log_probs"] = topk_log_probs  # (bsz, seqlen, topk)
                tensors["ref_topk_indices"] = topk_indices  # (bsz, seqlen, topk)
